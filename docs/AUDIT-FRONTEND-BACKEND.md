...
| 8 | **Sync Offline (passagens)** | `syncEngine.ts` + `SyncEngine.ts` + `offlineSync.ts` | `efvm360_sync_queue`, `efvm360_sync_status`, IndexedDB | `POST /sync/passagens`, `GET /sync/status`, `GET /sync/conflicts` | :warning: Tres implementacoes de sync existem. IndexedDB store esta funcional. Backend tem controller completo. Falta ativacao do circuito. |
