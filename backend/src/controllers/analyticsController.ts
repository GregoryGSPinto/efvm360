  ...
  try {
    ...
    for (const yard of yards) {
      for (let i = 0; i < 90; i++) {
        ...
        const compliance = 75 + Math.random() * 23;
        const fiveS = 3.2 + Math.random() * 1.6;
        const handovers = 5 + Math.floor(Math.random() * 11);
    ...
    res.status(500).json({ error: 'Erro interno ao atualizar analytics' });
  }
};
...
export const dashboardSupervisor = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const last30 = await DailyHandoverStats.findAll({
      where: { yard_code: yard, stat_date: { [Op.gte]: thirtyDaysAgo } },
      order: [['stat_date', 'ASC']],
    });

    const yardCompliance = await YardCompliance.findOne({ where: { yard_code: yard } });
    ...
    res.json({
      ...
      complianceTrend: last30.map(s => ({
        date: s.stat_date, compliance: s.compliance_rate,
        handovers: s.total_handovers, anomalies: s.anomalies_count,
      })),
      yardSummary: yardCompliance ? {
        compliance30d: yardCompliance.compliance_30d, trend: yardCompliance.trend,
        handovers30d: yardCompliance.handovers_30d, anomalies30d: yardCompliance.anomalies_30d,
      } : null,
    });
  } catch (err: unknown) {
    res.status(500).json({ error: 'Erro interno ao obter dashboard supervisor' });
  }
};
...
export const dashboardCoordenador = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const operators = await OperatorPerformance.findAll({
      where: { yard_code: { [Op.in]: yards }, period_month: { [Op.gte]: thirtyDaysAgo } },
    ...
    res.json({
      ...
      comparison: yardCompliance.map(yc => ({
        yard: yc.yard_code, compliance: yc.compliance_30d, trend: yc.trend,
        handovers: yc.handovers_30d, anomalies: yc.anomalies_30d, avgResolutionHours: yc.avg_resolution_hours,
      })),
      todayByYard: todayStats.map(s => ({
        yard: s.yard_code, handovers: s.total_handovers, compliance: s.compliance_rate,
        fiveS: s.five_s_avg_score, anomalies: s.anomalies_count,
      })),
      topOperators: operators.map(op => ({
        matricula: op.matricula, yard: op.yard_code, handovers: op.handovers_completed,
        fiveS: op.avg_five_s_score, anomalies: op.anomalies_reported,
      })),
    });
  } catch (err: unknown) {
    res.status(500).json({ error: 'Erro interno ao obter dashboard coordenador' });
  }
};
...
export const dashboardGerente = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const allYards = await YardCompliance.findAll({ order: [['compliance_30d', 'DESC']] });
    ...
    res.json({
      ...
      yardRanking: allYards.map(yc => ({
        yard: yc.yard_code, compliance: yc.compliance_30d, trend: yc.trend,
        handovers: yc.handovers_30d, anomalies: yc.anomalies_30d,
      ...
      slaAlerts: allYards
        ...
        .map(yc => ({
          yard: yc.yard_code, compliance: yc.compliance_30d,
          gap: parseFloat((TARGET_COMPLIANCE - Number(yc.compliance_30d)).toFixed(2)),
        })),
    });
  } catch (err: unknown) {
    res.status(500).json({ error: 'Erro interno ao obter dashboard gerente' });
  }
};
...
export const trends = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const stats = await DailyHandoverStats.findAll({ where, order: [['stat_date', 'ASC']] });

    res.json({
      period: `${periodDays}d`,
      yard: yard || 'all',
      data: stats.map(s => ({
        date: s.stat_date, yard: s.yard_code, compliance: s.compliance_rate,
        handovers: s.total_handovers, fiveS: s.five_s_avg_score, anomalies: s.anomalies_count,
      })),
    });
  } catch (err: unknown) {
    res.status(500).json({ error: 'Erro interno ao obter tendências' });
  }
};
...
export const operators = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const ops = await OperatorPerformance.findAll({
      where, order: [['handovers_completed', 'DESC']], limit: 50,
    });

    res.json({
      operators: ops.map(op => ({
        matricula: op.matricula, yard: op.yard_code, period: op.period_month,
        handovers: op.handovers_completed, fiveS: op.avg_five_s_score,
        anomalies: op.anomalies_reported, escalations: op.escalations_received,
        avgMinutes: op.avg_handover_minutes,
      })),
    });
  } catch (err: unknown) {
    res.status(500).json({ error: 'Erro interno ao obter desempenho de operadores' });
  }
};
